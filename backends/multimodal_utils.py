...
Compatible models - InternVL2, Nvidia NVLM
"""
...
def split_model(model_name):
    ...
    Args:
        model_name (str): The name of the model to be split. 
                          ...
                          'InternVL2-40B', 'InternVL2-Llama3-76B', 'NVLM-D-72B'
    ...
    num_layers = {
        ...
        'InternVL2-26B': 48, 'InternVL2-40B': 60, 'InternVL2-Llama3-76B': 80, 'NVLM-D-72B': 80}[model_name]
...
def build_transform_nvlm(input_size):
    MEAN, STD = IMAGENET_MEAN, IMAGENET_STD
    transform = T.Compose([
        T.Lambda(lambda img: img.convert('RGB') if img.mode != 'RGB' else img),
        T.Resize((input_size, input_size), interpolation=InterpolationMode.BICUBIC),
...
def find_closest_aspect_ratio_nvlm(aspect_ratio, target_ratios, width, height, image_size):
    best_ratio_diff = float('inf')
    best_ratio = (1, 1)
    area = width * height
    for ratio in target_ratios:
...
def dynamic_preprocess_nvlm(image, min_num=1, max_num=12, image_size=448, use_thumbnail=False):
    orig_width, orig_height = image.size
    aspect_ratio = orig_width / orig_height
    ...
    # find the closest aspect ratio to the target
    target_aspect_ratio = find_closest_aspect_ratio_nvlm(
        aspect_ratio, target_ratios, orig_width, orig_height, image_size)
...
def load_image_nvlm(image_file, input_size=448, max_num=12):
    image = Image.open(image_file).convert('RGB')
    transform = build_transform_nvlm(input_size=input_size)
    images = dynamic_preprocess_nvlm(image, image_size=input_size, use_thumbnail=True, max_num=max_num)
    pixel_values = [transform(image) for image in images]
    pixel_values = torch.stack(pixel_values)
    return pixel_values
...
def generate_nvlm_response(**response_kwargs) -> str:
    """Generates a response from the InternVL2 model based on the provided messages and configuration.
    ...
    images = get_nvlm_image(messages=messages).to(torch_dtype)
    history, question = generate_history_internvl2(messages=messages)
    if not history:
        history = None
    generation_config = dict(max_new_tokens=max_tokens, do_sample=True)
